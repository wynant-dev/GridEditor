...
void main() {
  group('ViewportCamera', () {
    test('zoomByAt keeps focal point fixed in screen space', () {
      const camera = ViewportCamera(offset: Offset(100, 50), zoom: 2.0);
      const focalPoint = Offset(200, 150);

      final worldBefore = (focalPoint - camera.offset) / camera.zoom;
      final zoomed = camera.zoomByAt(1.5, focalPoint);
      final screenAfter = worldBefore * zoomed.zoom + zoomed.offset;
