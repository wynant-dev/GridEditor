...
class GridInteractionController {
  ...
  void _resolveDragEnd() {
    ...
    if (editorController != null) {
      final placement =
          editorController.engine.placementById(session.placementId);
      if (placement != null) {
        final error = editorController.engine.placementError(
          catalogItemId: placement.catalogItemId,
          originRow: session.currentRow,
          originCol: session.currentCol,
          ignorePlacementId: session.placementId,
        );
        if (error == null) {
